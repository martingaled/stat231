...
SST <- sum(y^2) - (sum(y))^2/n  # Total sum of squares
SSRes <- SST - Sxy^2/Sxx                # Regression sum of squares
hatbeta1<-Sxy/Sxx
hatbeta0<-mean(y)-hatbeta1*mean(x)
hatsigma2<-SSRes/(n-2)  # estimate of variance of error term

errorbeta1<-sqrt(hatsigma2/Sxx)  # standard error of beta1
errorbeta0<-sqrt(hatsigma2*(1/n + (mean(x))^2/Sxx))  # standard error of beta0
...
(1 - pt(abs(tvalue), df = n-2))

qt(0.975, df = n-2)  # critical value for 95% confidence interval
qnorm(0.975)


# testing hypothesis beta0 = 0
(tvalue0<-hatbeta0/errorbeta0)

# p-value for two tail test
(2*(1 - pt(abs(tvalue0), df = n-2)))

# plotting tdistribution
...
```{r income_data}
income_data <- read.csv("incomedata.csv")
head(income_data)
...
se <- summary(model)$coefficients[2, 2]  # standard error of beta1
beta1 <- summary(model)$coefficients[2, 1]  # estimate of beta1
n <- nrow(income_data)  # number of observations
t_value <- qt(0.975, df = n - 2)  # critical value for 95% confidence interval
lower_bound <- beta1 - t_value * se
upper_bound <- beta1 + t_value * se
cat("95% confidence interval for beta1: [", lower_bound, ", ", upper_bound, "]\n")
...
abline(model$coefficients[1],model$coefficients[2], col = "red", lty = "dashed")
model <- lm(table.b1$y ~ table.b1$x8)
summary(model)
summary(aov(model))
# confidence interval for beta1
```
...
plot(x1, y)
plot(x2, y)
x <- matrix(rbind(rep(1, length(y)),x1, x2), byrow = TRUE, ncol = 3)
# bind by row
